...
export const CardsPokemons = (data) => {
  ...
  data.map((pokemon) => {
    ...
    const templateFigure = ` <figure class=${classCustomType} id=${pokemon.id}>
      <img src=${pokemon.image} alt=${pokemon.name} class="esconder" />
      <h3>${pokemon.name}</h3>
     ...
     */
    addListeners(data);
    addListenersPresionar();
  });
};

const addListenersPresionar = () =>{
  const figurePress = document.getElementById(`${pokemon.id}`)
  figurePress.addEventListener("click", () =>{
  })
